...
## 
lm_P_C <-  brm(dist ~ 1 + (1|part) + (1|cond),
               data = dist_from_mean,
               family = exponential(),
...
coefs_norm_P_C_part <- slice(dplyr::select(coefs_P_C, center, lower, upper), 6:25)+  0.48 #(dplyr::select(coefs,center),1)
coefs_norm_P_C_cond <- slice(dplyr::select(coefs_P_C, center, lower, upper), 2:5)+  0.48 #(dplyr::select(coefs,center),1)
